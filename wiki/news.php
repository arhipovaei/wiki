<?php
	...
	$title='Новости';
	$table='news';
	if (!in_array($_SESSION['level'], array(10, 5, 2, 1))) { // доступ разрешен только группе пользователей
		header("Location: login.php"); // остальных просим залогиниться
		exit;
	};
	$edit=in_array($_SESSION['level'], array(10, 5));
	$param_keys=array('dt', 'note'); // названия полей в таблице БД
	$param_str=array('Дата', 'Пояснение'); // названия столбцов в таблице для отображения
	$param_ext=array(0, 0); // поля для select
	$param_need='note'; // обязательные поля, без которых не сохранять данные
	$dates=array('dt'); // поля типа "дата"
	$textarea=array('note'); // поля типа "textarea"
?>
<!DOCTYPE html>
<html lang="ru">
<head>
...
<?php
	...
	$query="
		SELECT
			DATE_FORMAT(`news`.`dt`, '%d.%m.%Y') AS 'Дата', `news`.`note` AS 'Пояснение'
			$admin_delete
			$admin_edit
		FROM
			`$table`
...
<?php
	...
	for($ind=0; $ind<count($param_keys); $ind++) {
		...
		elseif (!$param_ext[$ind]) { // обычное поле input
			...
			<div class="form-field">
				<label for="'.$param_keys[$ind].'" class="my_label">'.$param_str[$ind].'</label>
				<select "'.$param_keys[$ind].'" name="'.$param_keys[$ind].'" class="my_input">
			';
